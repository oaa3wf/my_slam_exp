...
#include "my_slam/scoped_timer.h"
#include "my_slam/features.h"
#include "my_slam/transformation_estimation_euclidean.h"
...
unsigned int Node::featureMatching(const Node* other, std::vector<cv::DMatch>* matches) const 
{
  ...
  if (ps->get<std::string> ("matcher_type") != "BRUTEFORCE_OPENCV" && ps->get<std::string> ("feature_extractor_type") == "ORB")
  {
    ...
    //matcher->match(feature_descriptors_, other->feature_descriptors_, *matches);
	  ...
	  else if ( ps->get<std::string> ("feature_extractor_type") == "FREAK")
  {
...
MatchingResult Node::matchNodePair(const Node* older_node)
{
  ...
  try{
    ...
    if (mr.all_matches.size() < min_matches){
        ...
        found_transformation = getRelativeTransformationPCL2(older_node,&mr.all_matches, mr.ransac_trafo, mr.rmse, mr.inlier_matches);//getRelativeTransformationPCL(older_node,&mr.all_matches, mr.ransac_trafo, mr.rmse, mr.inlier_matches); 
        //Statistics  
        float nn_ratio = 0.0;
...
bool Node::getRelativeTransformationPCL2(const Node* earlier_node,
                                       std::vector<cv::DMatch>* initial_matches,
                                       Eigen::Matrix4f& resulting_transformation,
                                       float& rmse, 
                                       std::vector<cv::DMatch>& matches) const
...
double inlier_error;
const unsigned int sample_size = 3;// chose this many randomly from the correspondences:
std::vector<cv::DMatch> inlier2 = sample_matches_prefer_by_distance(sample_size, *matches_with_depth); 
const float max_dist_m = ParameterServer::instance()->get<double>("max_dist_for_inliers");
unsigned int min_inlier_threshold = (unsigned int) ParameterServer::instance()->get<int>("min_matches");
...
inliers1cloud->resize(initial_matches->size());
inliers2cloud->resize(initial_matches->size());
for(unsigned int i=0; i<initial_matches->size(); ++i)
{
    ...
    if(isnan(from(2)) || isnan(to(2)))
      continue;
		...
		(*inliers1cloud)[i].x = from(0);
		(*inliers1cloud)[i].y = from(1);
		(*inliers1cloud)[i].z = from(2);
		(*inliers2cloud)[i].x = to(0);
		(*inliers2cloud)[i].y = to(1);
		(*inliers2cloud)[i].z = to(2);

 
}

	if(inliers1cloud->size() >=3 && inliers1cloud->size() == inliers2cloud->size())
	{
		...
		std::vector<int> source_indices (inliers2cloud->size());
		std::vector<int> target_indices (inliers1cloud->size());

		// Copy the query-match indices
		for (int i = 0; i < (int)inliers1cloud->size(); ++i)
		{
			source_indices[i] = i;
			target_indices[i] = i;
		}
		...
		model.reset(new pcl::SampleConsensusModelRegistration<pcl::PointXYZ>(inliers2cloud, source_indices));
		// Pass the target_indices
		model->setInputTarget (inliers1cloud, target_indices);
		...
		if(sac.computeModel())
		{
			...
			if (inliers.size() >= 3)
			{
				...
				resulting_transformation.row (0) = model_coefficients.segment<4>(0);
				resulting_transformation.row (1) = model_coefficients.segment<4>(4);
				resulting_transformation.row (2) = model_coefficients.segment<4>(8);
				resulting_transformation.row (3) = model_coefficients.segment<4>(12);

				resulting_transformation = resulting_transformation.inverse().eval();
				...
				computeInliersAndError(*initial_matches, resulting_transformation, 
                           ...
                           earlier_node->feature_locations_3d_, //earlier_node->feature_depth_stats_, 
                           min_inlier_threshold, //break if no chance to reach this amount of inliers
                           inlier2, inlier_error, max_dist_m*max_dist_m);


			assert(inlier_error>=0);
      matches.assign(inlier2.begin(), inlier2.end());
      rmse = inlier_error;
			std::cout<< "matches :" << matches.size() <<std::endl;
			...
			return (matches.size() >= min_inlier_threshold);
				...
				//std::cerr("RANSAC inliers=%d/%d", (int)inliers.size(), (int)inliers1cloud->size());

				//return transform.inverse(); // inverse to get actual pose transform (not correspondences transform)
			}
			else
			{
				ROS_INFO("RANSAC: Model with inliers < 3");
				return false;
			}
		}
		else
		{
			ROS_INFO("RANSAC: Failed to find model"); 
			return false;
		}
	}
