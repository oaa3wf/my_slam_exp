...
optimizer_skip_step_(1),sequential_edges(0),loop_closures_edges(0),min_matches_(20)
{
	...
	backend = "pcg";
	pose_relative_to_ = "largest_loop";
                                   ...
                                   bool& edge_to_keyframe)///Output:contains the best-yet of the pairwise motion estimates for the current node
{
  ...
  //end: Initial Comparison ######################################################################
    ...
    int  seq_cand = ps->get<int>("predecessor_candidates") - 1; //minus one, because the first predecessor has already been checked
...
double GraphManager::optimizeGraphImpl(double break_criterion)
{
  ...
  else //Got the lock
  {
    ...
    fixationOfVertices(ps->get<std::string>("pose_relative_to"), optimizer_, graph_, camera_vertices, earliest_loop_closure_node_); 
